...
**Example:**
```
oaToken: "<your-oa-id>:<your-oa-secret>"
```

> **Note:** There is no `Authorization` header. The token is always passed as a body field named `oaToken`.

> **Getting a token:** every example below uses the `<your-oa-id>:<your-oa-secret>` placeholder. Request credentials for your Official Account from the GTalk team, then substitute both parts. Treat the secret like a password — never commit it or paste it into a shared document.
...
| `webhookResponseTimeoutInSecond`     | integer | ❌       | Timeout in seconds to wait for a response from the webhook (default: 10) |
...
--data '{
    ...
    "processingConfig": {
        "webhook": {
            "enabled": true,
            "webhookURL": "https://your-server.example.com/gtalk-webhook",
...
```json
{
    "data": {
        ...
        "PresignedURL": "https://s3-sgn10.fptcloud.com/gtalk-file-test/2026/01/12/414416020666490880/9340a3ae-cb62-4544-9dcc-c2162269f347?X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Credential=<redacted>&X-Amz-Date=20260422T075034Z&X-Amz-Expires=3600&X-Amz-SignedHeaders=host&x-id=GetObject&X-Amz-Signature=<redacted>"
    },
    "errorCode": "success",
    "error": {}
}
